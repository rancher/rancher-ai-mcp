...
func TestAddTools(t *testing.T) {
	cs, cleanup := startMCPServer(t, NewTools(client.NewClient(true), "not-used-in-test", false))
	defer cleanup()

	toolsResult, err := cs.ListTools(context.Background(), &mcp.ListToolsParams{})
	assert.NoError(t, err)
	assert.Len(t, toolsResult.Tools, 4, "incorrect number of RBAC tools registered")
...
func TestAddToolsReadOnly(t *testing.T) {
	cs, cleanup := startMCPServer(t, NewTools(client.NewClient(true), "not-used-in-test", true))
	defer cleanup()

	toolsResult, err := cs.ListTools(context.Background(), &mcp.ListToolsParams{})
	assert.NoError(t, err)
	assert.Len(t, toolsResult.Tools, 4, "incorrect number of RBAC tools registered in read-only mode")
	for _, tool := range toolsResult.Tools {
		assert.Equal(t, toolsSet, tool.Meta[toolsSetAnn])
	}
}
