...
func (t *Tools) listProjectRoleTemplateBindings(ctx context.Context, toolReq *mcp.CallToolRequest, params listPRTBParams) (*mcp.CallToolResult, any, error) {
	zap.L().Debug("listProjectRoleTemplateBindings called", zap.String("cluster", params.Cluster), zap.String("project", params.Project), zap.String("user", params.User))

	namespace := ""
	if params.Project != "" {
		projectBackingNamespace := params.Cluster + "-" + params.Project
	...
	prtbs = filterPRTBsByCluster(prtbs, params.Cluster)
